...
*/
// Cada vez que se incluya nuevo codigo debemos eliminar el exe porque sino no actualiza
...
namespace MouseMover
{
    public class MouseSimulator
    {
        ...
        // Con un poco de ingenieria inversa y robando codigo he conseguido enviar texto desde c#
        //https://docs.microsoft.com/es-es/dotnet/api/system.windows.forms.sendkeys.send?view=windowsdesktop-6.0
        //https://stackoverflow.com/questions/825651/how-can-i-send-the-f4-key-to-a-process-in-c
        // con el send da fallo pero el sendwait funciona
        static void keyboardWrite(string keys)
        {
        ...
        static void keyboardLeft(int repeat)
        {
            INPUT keyboardInput = new INPUT();
            keyboardInput.type = SendInputEventType.InputKeyboard;
            if (repeat>1)
            {
                System.Windows.Forms.SendKeys.SendWait("{LEFT " + repeat + "}");
            }
            else
            {
        ...
        static void keyboardRight(int repeat)
        {
            INPUT keyboardInput = new INPUT();
            keyboardInput.type = SendInputEventType.InputKeyboard;
            if (repeat>1)
            {
                System.Windows.Forms.SendKeys.SendWait("{RIGHT " + repeat + "}");
            }
            else
            {
       ...
       static void PrintHelp() {
            ...
            Console.WriteLine("keyboardWrite TEXT - output text");
            Console.WriteLine("");
        ...
        public static void Main(String[] args) {
            ...
            else if (args[0].ToLower() == "keyboardwrite")
            {
                CheckArgs(args);
                string keys_input = args[1];
                keyboardWrite(keys_input);
            }

            else if (args[0].ToLower() == "keyboardenter")
            {
                keyboardEnter();
            }
            else if (args[0].ToLower() == "keyboarddel")
            {
                keyboardDel();
            }
            else if (args[0].ToLower() == "keyboardleft")
            {
                CheckArgs(args);
                int repeat = int.Parse(args[1]);
                keyboardRight(repeat);
            }
            else if (args[0].ToLower() == "keyboardright")
            {
                CheckArgs(args);
                int repeat = int.Parse(args[1]);
                keyboardRight(repeat);
